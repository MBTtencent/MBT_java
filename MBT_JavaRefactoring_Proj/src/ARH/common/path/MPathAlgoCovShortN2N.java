/*
 * FileName: MPathAlgoCovShortN2N.java
 * 
 * Description: create MPathAlgoCovShortN2N class
 * 
 * History:
...
/**
 * this class is used to generate shortest paths from each start
 * state to each stop state
 * @author SHUYUFANG
 * @version 1.2
 */
public class MPathAlgoCovShortN2N extends MPathAlgo
{
    /** shortest distance list to get to states */
    private ArrayList<Integer> stateDistList;
    
    /** minimum distance from start state to other states in one round */
    private ArrayList<Integer> minDistList;
     ...
     * constructor
     */
    public MPathAlgoCovShortN2N()
    {
        super();
        this.stateDistList = new ArrayList<Integer>();
        this.minDistList = new ArrayList<Integer>();
    ...
    /**
     * reset minimum distance to be maximum value
     */
    private void resetMinDistList()
    {
        for (int i = 0; i < this.minDistList.size(); ++i)
        {
            this.minDistList.set(i, MPathConfig.MAX_PATH_DIST);
        }
    }
    ...
    private void deleteLongPath()
    {
        for (int i = this.tmpStatePathLists.size() - 1; i >= 0; --i)
        {
            ...
            // the path is not one of the shortest
            if (this.minDistList.get(endStateNum) < pathLength)
            {
    ...
    /**
     * generate shortest STATE TO STATE paths
    ...
    private void dfs(int curState, int curPos)
    {
        ...
        {
            if (this.minDistList.get(this.statePath.get(curPos - 1)) 
                    >= this.statePath.size())
            {
                this.minDistList.set(this.statePath.get(curPos - 1), 
                        this.statePath.size());
